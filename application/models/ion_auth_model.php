...
class Ion_auth_model extends CI_Model
{
	...
	public function admin_change_password($member_id, $password)
	{
		...
		$this->db->update($this->tables['users'], $data, array('id' => $member_id));
	...
	public function mobile_login($identity, $password)
	{
		if (empty($identity) || empty($password) || !$this->identity_check($identity)) {
			return array('error' => 'Username and Password is required');
		}
		...
		if ($query->num_rows() == 1) {
			...
			if ($result->password === $password) {
				...
				$guid = md5(uniqid());
				$guid_array = array('mobile_api' => $guid);

				$this->db->where('id', $result->id)->update('users', $guid_array);
			...
			return array('error' => 'Username could not be found');
		}
	}
	...
	public function get_clients($group = false, $limit = NULL, $offset = NULL, $trainer = NULL, $status = NULL)
	{
		$this->db->select(
			array(
				$this->tables['users'] . '.*',
				$this->tables['meta'] . '.*',
				$this->tables['groups'] . '.name AS ' . $this->db->protect_identifiers('group'),
				'trainer_client_groups.title AS trainer_group_title',
				$this->tables['groups'] . '.description AS ' . $this->db->protect_identifiers('group_description'),
				'trainer_clients.*'
			)
		);
		...
		$this->db->join($this->tables['meta'], $this->tables['users'] . '.id = ' . $this->tables['meta'] . '.' . $this->meta_join, 'left');
		$this->db->join($this->tables['groups'], $this->tables['users'] . '.group_id = ' . $this->tables['groups'] . '.id', 'left');
		$this->db->join('trainer_clients', 'users.id = trainer_clients.client_id', 'left');
		$this->db->join('trainer_client_groups', 'trainer_clients.trainer_group_id = trainer_client_groups.id', 'left');

		if (is_string($trainer)) {
			$this->db->where($this->tables['users'] . ".id IN (SELECT client_id FROM trainer_clients WHERE trainer_id ='" . $trainer . "')");
		}
	...
	public function get_trainer($user_id = false)
	{
		...
		$this->db->join($this->tables['meta'], $this->tables['users'] . '.id = ' . $this->tables['meta'] . '.' . $this->meta_join);
		$this->db->join($this->tables['groups'], $this->tables['users'] . '.group_id = ' . $this->tables['groups'] . '.id');
		...
		$this->db->where($this->tables['users'] . ".id IN (SELECT trainer_id FROM trainer_clients WHERE client_id ='" . $user_id . "')");
	...
	{
		$this->db->trans_begin();
		$workouts = $this->db->select('user_workouts.id')->where('user_id', $id)->get('user_workouts')->result_array();
		...
		$stats = $this->db->select('user_stats.id')->where('user_id', $id)->get('user_stats')->result_array();
		foreach ($stats as $stat) {
			$this->db->delete('user_stats_values', array('stat_id' => $stat['id']));
			$this->db->delete('user_stats', array('id' => $stat['id']));
		}
		$this->db->delete('user_available_exercises', array('user_id' => $id));
		$this->db->delete('trainer_clients', array('trainer_id' => $id));
		$this->db->delete('trainer_clients', array('client_id' => $id));
		$this->db->delete('trainer_workouts', array('user_id' => $id));
		$this->db->delete('user_workout_stats', array('user_id' => $id));
		$this->db->delete('user_weather', array('user_id' => $id));
		$this->db->delete('user_progressions', array('user_id' => $id));
