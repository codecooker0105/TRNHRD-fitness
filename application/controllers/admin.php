...
class Admin extends Controller {
	...
	{	
		...
		else
		{
			...
			if ($this->ion_auth->update_user($this->input->post('user_id'), $user_values))
			{ //if the password was successfully changed
	...
	{	
		...
		else
		{
			...
			if ($this->ion_auth->admin_change_password($this->uri->segment(3),$this->input->post('password')))
			{ //if the password was successfully changed
	...
	{	
		...
		{ //display the form
			...
			$this->data['exercise_id'] = array('name' => 'exercise_id',
				'id' => 'exercise_id',
				'type' => 'hidden',
				'value' => $exercise->id,
			);
	...
	{	
		...
		{ //display the form
			...
			$this->data['plan_id'] = array('name' => 'plan_id',
				'id' => 'plan_id',
				'type' => 'hidden',
				'value' => $plan->id,
			);
	...
	{	
		...
		{ //display the form
			...
			$this->data['workout_id'] = array('name' => 'workout_id',
				'id' => 'workout_id',
				'type' => 'hidden',
				'value' => $workout->id,
			);
	...
	function create_member()
	{
		...
		if ($this->form_validation->run() == true && $user_id = $this->ion_auth->register($username, $password, $email, $additional_data,$group_name))
		{ //check to see if we are creating the user
			...
			if($this->input->post('trainer') != ''){
				$insert_data = array('client_id' => $user_id,
