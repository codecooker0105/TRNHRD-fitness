...
// 			$this->load->view('base/footer');
	...
	{  
       if (!$this->ion_auth->logged_in()) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $insert_data = array('stat_id' => $this->input->post('id'),
             				        'date_taken' => date('Y/m/d',strtotime($this->input->post('date_taken'))),
									'stat_value' => $this->input->post('current_value')
        				       );
			$select_data = array('user_id' => $this->session->userdata('user_id'),
	...
	{  
       if (!$this->ion_auth->logged_in() || !$this->ion_auth->is_group('trainers')) {
        ...
        if ($error_message == '' && $this->form_validation->run() == true) { //check to see if we are creating the user
            ...
            $this->data['email_message']             = array('name'    => 'email_message',
		                                              'id'      => 'email_message',
		                                              'type'    => 'textarea',
	...
	?><p>The new workout(s) have been added to <?php if($user_type  == 'group'){ ?><?=$group->title?><?php }else{ ?><?=$user->first_name?> <?=$user->last_name?>'s<?php } ?> workout log. You may close this dialog and add this workout to another date or client or choose a destination below.</p>
		<ul>
			<li><a href="/member">Dashboard</a></li>
			<?php if($user_type  != 'group'){ ?>
            	<li><a href="/member/client_log_book/<?=$user->id?>"><?=$user->first_name?> <?=$user->last_name?>'s Workout Log</a></li>
            <?php } ?>
	...
	function get_admin_exercise_type(){
		...
		if($e_type){ ?>
        	<li class="category"><div class="ui-state-default ui-corner-all move"><span class="ui-icon ui-icon-arrow-4 move"></span></div><input type="hidden" value="<?=$e_type->id?>" name="category_id" class="category_id" /><a href="#" class="workout_category_title"><?=$e_type->title?></a>
			<?php
		}
	}
	...
	function get_exercise_type(){
		...
		$e_type = $this->crud->retrieve(array('id' => $id))->row();
		if($e_type){ ?>
        	<li class="category"><div class="ui-state-default ui-corner-all move"><span class="ui-icon ui-icon-arrow-4 move"></span></div><a href="#" class="workout_category_title"><?=$e_type->title?></a><?
                ?><ul class="workout_exercises">                    
                    <li class="exercise_type">
                        ...
                        <tr <?php if($set == 3){ ?> class="bottom" <?php } ?>>
                            ...
                            <td><span class="set_number"><?=$set?></span><input name="sets[]" type="hidden" value="<?=$set?>" /></td>
                            <td><select name="reps[]" class="reps">
                            <?php for($x=1;$x<=30;$x++){ ?>
                                <option value="<?=$x?>" <?php if(10 == $x){ ?> selected="selected" <?php } ?>><?=$x?></option>
                            <?php } ?>
                            </select>
                                <select name="time[]" class="time">
								<?php for($x=15;$x<=300;$x+=15){ ?>
                                    <option value="<?=$x?>" <?php if(30 == $x){ ?> selected="selected" <?php } ?>><?=secToMinute($x)?></option>
                                <?php } ?>
                                </select></td>
                            <td class="right">
                            <select name="rest[]" class="rest">
                            <?php for($x=0;$x<=300;$x+=15){ ?>
                                <option value="<?=$x?>" <?php if(30 == $x){ ?> selected="selected" <?php } ?>><?=$x?></option>
                            <?php } ?>
                            </select></td>
                            <td class="right">
                            	<span class="weight_input_box"><input type="text" name="weight[]" class="weight" value="" /> lbs.</span>
                        ...
                        <tr>
                            <td colspan="6" class="left">
                                ...
                                <a href="#" class="add_set">Add Set</a> | <a href="#" class="remove_set">Remove Set</a> | <a href="#" class="select_exercise<?=$e_type->id?>" id="exercise_10000">Select Exercise</a> | <a href="#" class="remove_exercise">Remove Exercise</a>
                            </td>
                        </tr>
                        </tbody>
                        </table>
	...
	function edit_photo()
	{
		...
		if($this->input->post('upload') != '' && $this->upload->do_upload('photo')){
			...
			$this->load->view('base/header',array('main_menu' => 'admin'));
			$this->load->view('base/integration_start');
			$this->load->view('member/edit_photo', $this->data);
			$this->load->view('base/integration_end');
			$this->load->view('base/footer');
	...
	function first_run()
	{
		...
		{ //check to see if the user is logging in
			...
			if($this->input->post('progression_plan_id') != ''){
				...
				if($progression_plan->days_week != count($this->input->post('workoutdays'))){
					...
					if ($this->ion_auth->update_user($user->id, $user_values))
					{ //if the login is successful
						//redirect them back to the home page
		...
		if ($this->form_validation->run() == false || $other_error == true)
		{  //the user is not logging in so display the login page
			//set the flash data error message if there is one
			if($other_error){
				$this->data['message'] = $error_message;
	...
	function confirm_trainer_request()
	{
		...
		{  //the user is not logging in so display the login page
			...
			$this->data['request_id'] = array('name' => 'request_id',
				'id' => 'request_id',
				'type' => 'hidden',
	...
	function client_calendar(){
		...
		$this->load->library('calendar', $prefs);
		$this->load->view('base/header',array('assets' => 'calendar'));
		$this->load->view('base/integration_start');
	...
	function log_book(){
		...
		$this->load->view('base/header',array('assets' => 'logbook'));
		$this->load->view('base/integration_start');
	...
	function register()
	{
		...
		if (!$captcha_error && $this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $group))
		...
		{ //display the create user form
			...
			$this->data['hpot'] = array('name' => 'hpot',
				'id' => 'hpot',
				'type' => 'hidden',
				'value' => ''
			);
