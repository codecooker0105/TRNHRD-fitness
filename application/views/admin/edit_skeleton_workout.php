<div class='mainInfo'>
    ...
    <?php echo form_open("admin/edit_skeleton_workout/".$this->uri->segment(3),'id="admin_skeleton_form"');?>
      ...
      <? echo form_multiselect('workout_progressions[]',$progression_options,$workout_progressions,'style="width:250px;height:200px;"'); ?>
      </p>
    ...
    <? foreach($workout_sections as $section){ ?>
        ...
        <input type="hidden" value="<?=$section['section_type_id']?>" name="section_id" class="section_id" />
            <? if($section['type'] == 'rest' || $section['type'] == 'active-rest'){ ?>
                <span class="rest"><?=$section['title']?></span>
            <? }else{ ?>
            ...
            <ul class="workout_categories">
                <? foreach($section['exercises'] as $exercise){ ?>
                	<li class="category"><div class="ui-state-default ui-corner-all move"><span class="ui-icon ui-icon-arrow-4 move"></span></div><input type="hidden" value="<?=$exercise['exercise_type_id']?>" name="category_id" class="category_id" /> <strong><?=$exercise['title']?></strong> - <a href="#" class="remove_exercise">Remove Exercise</a>
                <? } ?>
            </ul>
    <? } ?>
    </ul>
    ...
    <p><?php echo form_submit('submit2', 'Submit','id="submit"');?></p>
    <?php echo form_close();?>
