...
<div id="generator_left">
    ...
    <input type="text" value="" id="date" class="date" /></p>
    <div id="week_days">
    <p><strong>Days of Week:<br /></strong>
		<? foreach($weekdays as $day => $checkbox){
			?> <?php echo form_checkbox($checkbox);?> <?=$weekday_title[$day]?> &nbsp&nbsp&nbsp&nbsp;<?
		} ?>
    </p>
    </div>
    ...
    <? foreach($equipment as $id => $title){
		?><?php echo form_checkbox($available_equipment[$id]);?> <?=$title?>&nbsp&nbsp&nbsp&nbsp;<?
	} ?>
    </p>
    
    <hr />
    <p>Select a progression type and workout to get started and a workout will be generated for you. You can always modify the workout afterwards.<br />
...
<div id="generator_right">
    <div class="right_block">
    	<div class="header"><h3 class="header_221">Clients</h3><a href="/member/clients" class="small_edit">Edit</a></div>
                        ...
                        ?><li><a href="#" class="level_title off"><span><?=$level['title']?></span></a>
                            <? if(isset($level['exercises'])){ ?>
                                ...
                                <? foreach($level['exercises'] as $exercise){
                                    ?><li class="exercise"><a href="/member/popup_video/<?=$exercise['id']?>" class="play-exercise"><?=$exercise['title']?></a></li><?
                                } ?>
                                </ul>
                            <? } ?></li><?
						}
...
<div id="dialog" title="Create new section">
	<form>
        ...
        <select name="section" id="section_dropdown" class="ui-widget-content ui-corner-all">
        	<? $result = mysql_query("SELECT * FROM skeleton_section_types ORDER BY title");
...
<div id="exercise_dialog" title="Create new exercise">
	...
	<fieldset>
        ...
        <select name="type" id="exercise_type_dropdown" class="ui-widget-content ui-corner-all">
        	...
        	<? $result = mysql_query("SELECT * FROM exercise_types ORDER BY title");
			while($row = mysql_fetch_assoc($result)){
...
<? $result = mysql_query("SELECT * FROM exercise_types");
	while($row = mysql_fetch_assoc($result)){ ?>
	<div id="dialog_<?=$row['id']?>" title="Select Exercise">
        <form>
            <h2>Exercise</h2>
            <ul style="list-style:none;">
                <? $result2 = mysql_query("SELECT * FROM exercises WHERE id IN(SELECT exercise_id FROM exercise_link_types WHERE type_id = '" . $row['id'] . "') ORDER BY title");
                while($row2 = mysql_fetch_assoc($result2)){
                    ?><li><input type="radio" name="exercise_id" value="<?=$row2['id']?>"><?=$row2['title']?> - <a href="/member/popup_video/<?=$row2['id']?>" class="play-exercise">View Video</a></li><?
                } ?>
            </ul>
        </form>
    </div>
...
<div id="equipment_dialog" title="Workout Reset Warning">
	<p>Changing the available equipment will reset the current workout and lose any unsaved data. If you wish to continue click on Reset Workout. Otherwise just close this warning or click 'Leave Equipment As Is'.</p>
