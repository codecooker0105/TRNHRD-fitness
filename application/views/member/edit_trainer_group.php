...
<p>Enter a title for the group and then select which clients will be a part off this group. Remember clients that are
    currently a part of another group will be removed that group if selected.</p>
...
<p>Experience Level:<br />
    <?php echo form_dropdown('exp_level_id', $experience_options, $experience_id, 'class="required smallsize"'); ?>
</p>
...
<table cellpadding="0" cellspacing="0" class="listing tablesorter" id="clients" width="100%">
    ...
    <?php foreach ($clients as $count => $user): ?>
        <tr class="<?= ($count % 2) ? 'odd' : 'even'; ?>">
            <td><input type="checkbox" name="clients[]" value="<?= $user->client_id ?>" <?php if (in_array($user->client_id, $group_clients)) { ?> checked="checked" <?php } ?> /></td>
            <td>
