    	...
    	<ul id="weather_tabs">
			...
			foreach($weathers as $zip => $weather){ ?>
				<li><a href="#" id="tab<?=$zip?>"  class="tab_link<? if($count == 0){ ?> on<? } ?>" ><span><?=substr($weather->loc->dnam,0,strripos($weather->loc->dnam,','))?></span></a></li>
			<? $count++;
			} ?>
        ...
        <div id="inner_weather">
            ...
            foreach($weathers as $zip => $weather){ 
                ?><div id="weather_tab<?=$zip?>"  class="tab<? if($count == 0){ ?> on<? } ?>" >
                    <h4><?=$weather->loc->dnam?> - <a class="confirmDeleteLink" href="<?=site_url('member/remove_weather/'.$zip)?>">Remove</a></h4>
                    <table width="100%">
                    <tr>
