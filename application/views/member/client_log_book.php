<div id="generator_left">
    ...
    <h1><span id="log_date"><?=$title?></span>: <?php if($workout){ ?><?=$workout['title']?><? } ?></h1>
    ...
    <? if($workout['trainer_id'] == $user->id){
		?><ul>
            ...
            <ul>
            <li><a href="/member/workout_generator/workout/<?=$workout['workout_id']?>">Edit Today's Workout</a></li>
            <? if($workout['end_date'] != ''){ ?>
            <li><a href="/member/workout_generator/trainer_workout/<?=$workout['trainer_workout_id']?>">Edit All Occurences of this workout</a></li>
            <? } ?>
            <li><a href="<?=$workout['workout_id']?>" class="confirmWorkoutDeleteLink">Delete Today's Workout</a></li>
            <? if($workout['end_date'] != ''){ ?>
            <li><a href="<?=$workout['user_id']?>-<?=$workout['trainer_workout_id']?>" class="confirmTrainerWorkoutDeleteLink">Delete Today's Workout and all occurences of this workout</a></li>
            <? } ?>
            </ul>
            </li>
            <? if($workout['trainer_group_id'] != ''){ ?>
                <li><strong>Group Options</strong><ul>
                <li><a href="/member/workout_generator/group_workout/<?=$workout['workout_id']?>">Edit Today's Workout for entire group</a></li>
                <? if($workout['end_date'] != ''){ ?>
                <li><a href="/member/workout_generator/trainer_group_workout/<?=$workout['trainer_workout_id']?>">Edit All Occurences of this workout for entire group</a></li>
                <? } ?>
                <li><a href="<?=$workout['workout_id']?>" class="confirmGroupWorkoutDeleteLink">Delete Today's Workout for entire group</a></li>
                <? if($workout['end_date'] != ''){ ?>
                <li><a href="<?=$workout['trainer_workout_id']?>" class="confirmTrainerGroupWorkoutDeleteLink">Delete Today's Workout and all occurences of this workout for entire group</a></li>
                <? } ?>
                </ul></li>
            <? } ?>
       	</ul><?
	...
	<? if($workout['created'] == 'true'){ ?>
    	<p><a target="_blank" href="/member/print_log_book<? if($this->uri->segment(3) != ''){ ?>/<?=$this->uri->segment(3)?>/<?=$this->uri->segment(4)?>/<?=$this->uri->segment(5)?><? } ?>">Printer Friendly Version</a></p>
                        ...
                        <ul class="type">
                        	<li class="exercise_type">
                                ...
                                <tr <? if($set == count($ex_sets)){ ?> class="bottom" <? } ?>>
                                    ...
                                    <td><?=$set?><input name="sets[]" type="hidden" value="<?=$set?>" /></td>
                                    ...
                                    <td class="right">
										<? if($exercise['weight_option'] == 'weighted'){ ?>
                                        	<input type="text" name="weight[<?=$set?>]" class="weight" value="<?=$weight?>" /> lbs.
                                ...
                                <tr>
                                	<td colspan="4" class="left">
                                        ...
                                        <input type="radio" name="difficulty" value="1" <? if($difficulty == 1){ ?> checked="checked" <? } ?> /> easy
                                        <input type="radio" name="difficulty" value="2" <? if($difficulty == 2){ ?> checked="checked" <? } ?> /> moderate
                                        <input type="radio" name="difficulty" value="3" <? if($difficulty == 3){ ?> checked="checked" <? } ?> /> difficult
                                        <input type="radio" name="difficulty" value="4" <? if($difficulty == 4){ ?> checked="checked" <? } ?> /> impossible
                                    </td>
...
<div id="generator_right">
    ...
    <? if($current_client){ ?>
		<? if($current_client->photo != ""){ ?>
            <img src="/images/member_photos/<? echo $current_client->photo; ?>" />
        <? }else{ ?>
    ...
    <?php if($past_workouts){
		...
		foreach($past_workouts as $workout){
			...
			?><li><a class="day" href="/member/client_log_book/<?=$current_client->id?>/<?=date('Y/m/d',strtotime($workout->workout_date))?>"><?=date('l jS',strtotime($workout->workout_date))?></a></li><?
		}
	} ?>
    </ul>
...
<div id="group_single_dialog" title="Confirmation Required" style="display:none;">
  Are you sure you want to remove this workout for today for the entire workout group? This will delete today's workout for everyone in this workout group. This can not be undone.
</div>

<div id="group_all_dialog" title="Confirmation Required" style="display:none;">
  Are you sure you want to remove this workout and all occurrences of this workout for the entire group? This will delete all occurences of this workout for everyone in this workout group. This can not be undone.
</div>