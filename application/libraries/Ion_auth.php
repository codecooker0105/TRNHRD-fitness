...
class Ion_auth
{
	...
	{
		if ($this->ci->ion_auth_model->admin_change_password($member_id, $password))
		{
			$this->set_message('password_change_successful');
			return TRUE;
		}
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ci->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			$user = $this->get_user_by_email($identity);  //changed to get_user_by_identity from email
	...
	{
		return $this->ci->ion_auth_model->mobile_login($identity, $password);
	}
	...
	{
		if ($this->ci->ion_auth_model->admin_login($identity, $password, $remember))
		{
			$this->set_message('login_successful');
			return TRUE;
		}
	...
	{
		return $this->ci->ion_auth_model->get_groups();
	}
	...
	public function get_clients($group_name=false, $limit=NULL, $offset=NULL, $trainer=NULL, $status=NULL)
	{
		return $this->ci->ion_auth_model->get_clients($group_name, $limit, $offset, $trainer, $status)->result();
	}

	public function get_trainer($user_id=FALSE)
	{
		return $this->ci->ion_auth_model->get_trainer($user_id)->row();
	}
