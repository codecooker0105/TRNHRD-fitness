...
$(document).ready(function() {
	...
	$('select#client').change(function(){
		if($(this).val() != ''){
			...
			$.post('/member/generator_get_client',{'id':$(this).val()},function(data){
				if(data.type == 'client'){
					if(data.photo != ''){
						$("#photo").html('<img src="/images/member_photos/' + data.photo + '" />');
					}else{
					...
					$("#photo").html('<h3>Clients in Group</h3><ul id="group_clients"></ul>');
					$.each(data.clients,function(index,value){
						$("#group_clients").append('<li>' + value.first_name + ' ' + value.last_name + '</li>');
					});
					equipment = data.available_equipment.split(',');
					$.each(equipment,function( index, value ){
						$("input#equipment" + value).attr('checked','checked');
	...
	$("input#generate").click(function(){
		if($("select.progression").val() == ''){
			...
			var equipment = $('.equipment:checked').map(function(i,n) {
				return $(n).val();
			...
			if($(this).val() != ''){
				$("ul#workout_list").load("/member/skeleton_json",{'id':$("select.skeleton").val(),'progression_id':$("select.progression").val(),'user_id':$("#client").val(),'available_equipment':equipment},function(){
	...
	$("#equipment_dialog").dialog({
		...
		buttons: {
			'Reset Workout': function() {
				$("ul#workout_list").html('');
				$('.skeleton_selector').find('option:first').attr('selected', 'selected').parent('select');
				$(this).dialog('close');
			},
	...
	$('#workout_generator_form').submit(function(){
		//alert($("#date").val());
		var querystring = 'action=save_workout&' + $("#workout_title").fieldSerialize() + '&' + $("#workout_id").fieldSerialize() + '&' + $("#group_workout_id").fieldSerialize() + '&' + $("#trainer_group_workout_id").fieldSerialize() + '&' + $("#trainer_workout_id").fieldSerialize() + '&' + $("#client").fieldSerialize() + '&' + $(".progression").fieldSerialize() + '&' + $(".skeleton").fieldSerialize() + '&date=' + $("#date").val();
		...
		if($("#date").val() == ''){
			...
			error = true;
			error_message += '<li>You must have at least 1 section to your workout or select a skeleton workout</li>';
		}else{
			$('#workout_list li.section').each(function(index){
				querystring += '&workout[' + index + '][]=' + $(this).children('input.section_id').fieldValue();
				if($(this).find('.section_rest') != undefined){
					querystring += '-' + $(this).find('.section_rest').val();
				}
				$(this).find('ul.workout_categories li ul li').each(function(index2){
					if(!error){
						if($(this).find('input.exercise_id').val() == ''){
							error = true;
							error_message += '<li>You have workout sections without exercises selected</li>';
						}
						var sets = '';
						var reps = '';
						var rest = '';
						var weight = '';
						var time = '';
						$(this).find('input.sets').each(function(index3){
							sets += $(this).val() + '|';
						});
						$(this).find('select.reps').each(function(index3){
							reps += $(this).val() + '|';
						});
						$(this).find('select.rest').each(function(index3){
							rest += $(this).val() + '|';
						});
						$(this).find('input.weight').each(function(index3){
							weight += $(this).val() + '|';
						});
						$(this).find('select.time').each(function(index3){
							time += $(this).val() + '|';
						});
						
						querystring += '&workout[' + index + '][][' + index2 + ']=' + $(this).find('input.category_id').val() + '-' + $(this).find('input.exercise_id').val() + '-' + sets + '-' + reps + '-' + rest + '-' + weight + '-' + time + '-' + $(this).find('select.set_type').val() + '-' + $(this).find('select.weight_option').val();
	...
	if($("#workout_id") != undefined && $("#workout_id").val() != ''){
		$.post("/member/get_workout_details_for_generator",
		{'id':$("#workout_id").val()},
			function(data){
		...
		$("ul#workout_list").load("/member/get_workout_for_generator",{'id':$("#workout_id").val()},function(){
			updateWorkoutListControls();			
		}).fadeIn();
	}
	
	if($("#group_workout_id") != undefined && $("#group_workout_id").val() != ''){
		$.post("/member/get_workout_details_for_generator",
		...
		$("ul#workout_list").load("/member/get_workout_for_generator",{'group_id':$("#group_workout_id").val()},function(){
			updateWorkoutListControls();			
		}).fadeIn();
	}
	
	if($("#trainer_workout_id") != undefined && $("#trainer_workout_id").val() != ''){
		$.post("/member/get_workout_details_for_generator",
			...
			function(data){
				if(data.success == 'true'){
					...
					if(data.details.end_date != ''){
						...
						$("#date").val(start_month + "/" + start_day + "/" + start_year + ' - ' + end_month + "/" + end_day + "/" + end_year);
					}else{
					...
					$("ul#workout_list").load("/member/get_workout_for_generator",{'id':data.details.workout_id},function(){
						updateWorkoutListControls();			
					}).fadeIn();
	...
	if($("#trainer_group_workout_id") != undefined && $("#trainer_group_workout_id").val() != ''){
		$.post("/member/get_workout_details_for_generator",
...
function updateWorkoutListControls(){
	...
	$("li.category").droppable({
		...
		drop: function( event, ui ) {
			$( this )
				.find( ".exercise_id" )
					.val( ui.draggable.find('a').attr('id') );
			$( this )
	...
	$("li.section").droppable({
		...
		drop: function( event, ui ) {
			clicked_item = $(this);
			$.post('/member/get_exercise_type',{exercise_id:ui.draggable.find('a').attr('id')},function(data){
