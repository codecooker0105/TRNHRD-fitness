...
$(document).ready(function() {
		...
		$("#single_dialog").dialog({
			...
			buttons : {
				"Yes, Please remove this workout" : function() {
					$.post("/member/remove_workout",
						{ id: targetUrl},
							function(data){
								if(data.success == 'true'){
									$('<h1>' + $("#log_date").html() + '</h1>').insertBefore("#logbook_container");
									$("#logbook_container").html('<p>Your client does not have any workouts on this day</p>');									
								}
	...
	$(".confirmGroupWorkoutDeleteLink").live('click',function(e) {
		...
		$("#group_single_dialog").dialog({
			...
			buttons : {
				"Yes, Please remove this workout" : function() {
					$.post("/member/remove_group_workout",
						{ id: targetUrl},
							function(data){
								if(data.success == 'true'){
									$('<h1>' + $("#log_date").html() + '</h1>').insertBefore("#logbook_container");
		...
		$("#all_dialog").dialog({
			...
			buttons : {
				"Yes, Please remove all occurences of this workout" : function() {
					$.post("/member/remove_trainer_workout",
						{ id: targetUrl},
							function(data){
								if(data.success == 'true'){
									$('<h1>' + $("#log_date").html() + '</h1>').insertBefore("#logbook_container");
	...
	$(".confirmTrainerGroupWorkoutDeleteLink").live('click',function(e) {
		...
		$("#group_all_dialog").dialog({
			...
			buttons : {
				"Yes, Please remove all occurences of this workout" : function() {
					$.post("/member/remove_trainer_group_workout",
						{ id: targetUrl},
							function(data){
								if(data.success == 'true'){
									$('<h1>' + $("#log_date").html() + '</h1>').insertBefore("#logbook_container");
