...
$(document).ready(function() {
	...
	$.post('/member/get_stats_chart', 'interval=weekly', function(stats_data) {
		stat_count = 0;
		$.each(stats_data, function(i, stat_type){
			...
			$.each(stat_type, function(j, stat_value){
				if(options.series[stat_count].pointStart == undefined){
					options.series[stat_count].pointStart = Date.parse(stat_value.week_start +' UTC');
				}
	...
	$("#chart_form").submit(function(){
		$.post('/member/get_stats_chart', 'interval=' + $("#interval").val(), function(stats_data) {
			if($("#interval").val() == 'daily'){
				...
				$.each(stats_data, function(i, stat_type){
					...
					$.each(stat_type, function(j, stat_value){
						if(options.series[stat_count].pointStart == undefined){
							options.series[stat_count].pointStart = Date.parse(stat_value.day +' UTC');
						}
						options.series[stat_count].name = stat_value.title;
						stat_totals.push(parseInt(stat_value.average.replace(',', ''), 10));
				...
				$.each(stats_data, function(i, stat_type){
					...
					$.each(stat_type, function(j, stat_value){
						if(options.series[stat_count].pointStart == undefined){
							options.series[stat_count].pointStart = Date.parse(stat_value.month_start +' UTC');
						}
						options.series[stat_count].name = stat_value.title;
						stat_totals.push(parseInt(stat_value.average.replace(',', ''), 10));
