...
$(document).ready(function() {
	...
	$('select#client').change(function(){
		...
		$.post('/member/get_client',{'id':$(this).val()},function(data){
			if(data.photo != ''){
				...
				$("ul#workout_list").load("/member/skeleton_json",{'id':$("select.skeleton").val(),'progression_id':$("select.progression").val(),'user_id':$("#client").val(),'available_equipment':equipment},function(){
					...
					$('.expand_collapse').click(function() {
						$(this).parent().children('ul').toggle('normal',function(){
							if($(this).parent().children('.expand_collapse').hasClass('ui-icon-circle-triangle-n')){
								$(this).parent().children('.expand_collapse').removeClass('ui-icon-circle-triangle-n');
								$(this).parent().children('.expand_collapse').addClass('ui-icon-circle-triangle-s');
							}else{
								$(this).parent().children('.expand_collapse').removeClass('ui-icon-circle-triangle-s');
								$(this).parent().children('.expand_collapse').addClass('ui-icon-circle-triangle-n');
							}						
						});					
					});
	...
	$("#sets_reps_dialog").dialog({
		...
		buttons: {
			'Update Sets & Reps': function() {	
				...
				$('#sets_' + select_item).parent().find('input.sets').val($('#sets_dropdown').val()); 
				$('#sets_' + select_item).parent().find('input.sets').val($('#reps_dropdown').val()); 
				$('#sets_' + select_item).parent().find('.sets_reps_time').html($('#sets_dropdown').val() + ' Sets x ' + $('#reps_dropdown').val() + ' Reps');  
				$('.play-exercise').colorbox();
				$(this).dialog('close');
			},
			'Close': function() {	
	...
	$('#workout_generator_form').submit(function(){
		alert($("#date").val());
		var querystring = 'action=save_workout&' + $("#workout_title").fieldSerialize() + '&' + $("#client").fieldSerialize() + '&' + $(".progression").fieldSerialize() + '&' + $(".skeleton").fieldSerialize() + '&date=' + $("#date").val();
			...
			$('#workout_list li.section').each(function(index){
				...
				$(this).find('ul.workout_categories li ul li').each(function(index2){
					if(!error){
						...
						querystring += '&workout[' + index + '][][' + index2 + ']=' + $(this).find('input.category_id').val() + '-' + $(this).children('input.exercise_id').val() + '-' + $(this).children('input.sets').val() + '-' + $(this).children('input.reps').val() + '-' + $(this).children('input.time').val();
					}
				});
			});
		}
