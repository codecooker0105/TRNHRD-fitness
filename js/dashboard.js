      ...
      $("#add_current_stat_dialog").dialog({
        ...
        buttons: {
          "Yes, Please update this stat": function () {
            ...
            if (current == "" || date == "") {
              $("#stat_message").html(
                "You must enter a value and selct a date"
              );
            } else {
              $.post(
                "/member/add_current_stat",
      ...
      $("#featured_exercise_dialog").dialog({
        ...
        buttons: {
          "Select this workout": function () {
            var current_workout = $(
              "#featured_exercise_upcoming_workouts"
            ).val();
            $("#featured_exercise_upcoming_workouts").attr(
              "disabled",
              "disabled"
            );
            $.post(
              "/member/get_similiar_workout_exercises",
                ...
                if (data.error == undefined) {
                  if (data.exercises != "none") {
                    ...
                    $.each(data.exercises, function (i, exercise) {
                      $("select#replace_exercise_id").append(
                        $("<option></option>")
                  ...
                  $("#featured_exercise_dialog").dialog("option", {
                    buttons: {
                      "Add Exercise to Workout": function () {
                        ...
                        var featured_choice = $(
                          "input[name=featured_option]:checked"
                        ).val();
                        $.post(
                          "/member/add_featured_exercise_to_workout",
                          {
                            exercise: exercise_id,
                            workout_id: current_workout,
                            choice: featured_choice,
                            uwe: uwe_id,
                            uws: uws_id,
                          },
                          function (data) {
